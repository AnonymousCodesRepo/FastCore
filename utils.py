...
def train_test_in_epoch(train_loader, network, criterion, optimizer, scheduler, epoch, args, rec, test_loader, test_iteration_interval, if_weighted: bool = False):
    """Train for one epoch on the training set"""
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    ...
    for i, contents in enumerate(train_loader):
        ...
        if i % test_iteration_interval == 0:
