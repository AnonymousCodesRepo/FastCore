...
class DeepFool(EarlyTrain):
    ...
    def deep_fool(self, inputs):
        ...
        # r_tot = np.zeros([sample_size, inputs.shape[1]])

        r_tot = np.zeros([sample_size, np.prod(inputs.shape[1:])])
        ...
        for i in range(self.max_iter):
            ...
            cur_inputs = cur_inputs + r_i.reshape([r_i.shape[0]] + list(original_shape))
